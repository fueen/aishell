...
(function (window, $, undefined) {
    ...
    //  取消由 coreUtil.delay / $.commons.delay 方法设置的函数延迟调用等待；该函数定义如下参数：
    //      id_of_delay: 必须。要取消的延迟等待函数调用的 handle 值（该值由方法 coreUtil.delay / $.commons.delay 返回）。
    //      undelayJust: 可选。Boolean 类型值，表示要取消的延迟函数是否为即时调用函数（即通过 coreUtil.delay / $.commons.delay 设定该函数时未传入 millisec 值或 millisec 值为 0。）；该参数默认为 false。
    ...
    //  如果 $.commons.supportsFullScreen 值为 true，则该属性获取浏览器的全屏 API 事件名称；如果 $.commons.supportsFullScreen 值为 false；则该属性返回 null/undefined。
